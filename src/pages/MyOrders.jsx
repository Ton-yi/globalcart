...
const ALL_COLUMNS = [
  ...
  { key: "order_status", label: "状态", visible: true },
  { key: "prepayment_amount", label: "金额", visible: true },
  { key: "weight_g", label: "重量", visible: true },
...
export default function MyOrders() {
  ...
  const renderCell = (col, order) => {
    ...
    switch (col.key) {
      ...
      case "product_name":
        return (
          <td key={col.key} className="px-4 py-3">
            <div
              className={`font-medium truncate max-w-xs text-sm ${isPaid ? "text-blue-600 hover:text-blue-800 cursor-pointer flex items-center gap-1" : "text-gray-900"}`}
              onClick={isPaid ? () => handleProductClick(order) : undefined}
            >
              {order.product_name}
              {isPaid && <ExternalLink className="w-3 h-3 flex-shrink-0 opacity-50" />}
            </div>
            {unread && <Badge className="bg-orange-100 text-orange-700 text-xs mt-0.5 animate-pulse">有新回复</Badge>}
      ...
      case "weight_g":
        return <td key={col.key} className="px-4 py-3 text-xs text-gray-500">{order.weight_g ? `${order.weight_g}g` : "-"}</td>;

      case "payment_due_date":
        return <td key={col.key} className="px-4 py-3 text-xs text-gray-500">{order.payment_due_date || "-"}</td>;

      case "product_description":
        return <td key={col.key} className="px-4 py-3 text-xs text-gray-500 max-w-[120px] truncate">{order.product_description || "-"}</td>;
      ...
      case "admin_note":
        return <td key={col.key} className="px-4 py-3 text-xs text-gray-500 max-w-[120px] truncate">{order.admin_note || "-"}</td>;

      case "user_note":
        return <td key={col.key} className="px-4 py-3 text-xs text-gray-500 max-w-[120px] truncate">{order.user_note || "-"}</td>;
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                {visibleCols.map(col => (
                  <th key={col.key} className="text-left px-4 py-3 text-xs font-medium text-gray-500 whitespace-nowrap">
                    {col.label}
                  </th>
                ))}
              ...
              {displayOrders.map(order => {
                ...
                return (
                  <tr key={order.id} className="hover:bg-gray-50">
                    {visibleCols.map(col => renderCell(col, order))}
