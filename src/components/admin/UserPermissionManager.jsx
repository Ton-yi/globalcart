...
export default function UserPermissionManager({ user, allRoles: allRolesProp, onClose }) {
  ...
  const previewRole = useMemo(
    () => previewRoleId ? loadedRoles.find(r => r.id === previewRoleId) : null,
    [previewRoleId, loadedRoles]
  );

  const previewPerms = useMemo(
    () => new Set(previewRole?.direct_permissions || []),
    [previewRole]
  );

  const handleApplyRolePreview = () => {
    if (!previewRole) return;
    setEffectivePerms(prev => {
      const next = new Set(prev);
      previewPerms.forEach(p => next.add(p));
      return next;
    });
  };
        ...
        <div className="space-y-5">
          ...
          <div>
            <Label className="text-xs text-gray-500 font-semibold block mb-2">
              分配角色（可多选）<span className="text-gray-400 font-normal ml-1">· 点击角色预览其权限配置</span>
            </Label>
            {loadedRoles.length === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-2">
                {loadedRoles.map(role => {
                  ...
                  const isPreviewing = previewRoleId === role.id;
                  return (
                    <button
                      key={role.id}
                      onClick={() => setPreviewRoleId(isPreviewing ? null : role.id)}
                      className={`px-3 py-2 rounded-lg border-2 text-left text-sm font-medium transition-colors flex items-center justify-between gap-1 ${
                        isPreviewing
                    ...
                    >
                      ...
                      <Eye className={`w-3.5 h-3.5 flex-shrink-0 ${isPreviewing ? 'text-purple-500' : 'text-gray-300'}`} />
                    </button>
                  );
                })}
              </div>
            ...
            {/* Role preview panel */}
            {previewRole && (
              <div className="mt-3 border border-purple-200 rounded-lg bg-purple-50 p-3">
                <div className="flex items-center justify-between mb-2 gap-2 flex-wrap">
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: previewRole.color || '#9ca3af' }} />
                    <span className="text-xs font-semibold text-purple-800">「{previewRole.name}」权限配置</span>
                    <Badge className="text-xs bg-purple-100 text-purple-700 border-purple-200">{previewPerms.size} 项权限</Badge>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
                      size="sm"
                      className="h-7 text-xs bg-purple-600 hover:bg-purple-700 gap-1"
                      onClick={() => { handleApplyRolePreview(); handleRoleToggle(previewRole.id); setPreviewRoleId(null); }}
                    >
                      <Download className="w-3 h-3" />套用此角色权限
                    </Button>
                    <button onClick={() => setPreviewRoleId(null)} className="text-purple-400 hover:text-purple-600 text-sm leading-none">✕</button>
                  </div>
                </div>
                {previewPerms.size === 0 ? (
                  <p className="text-xs text-purple-400">此角色暂无配置权限</p>
                ) : (
                  <PermissionGrid
                    selected={[...previewPerms]}
                    onToggle={() => {}}
                    accentColor="purple"
                    disabled
                  />
