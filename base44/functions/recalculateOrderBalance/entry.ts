...
Deno.serve(async (req) => {
  try {
    ...
    if (!['prepay', 'deferred'].includes(order.payment_mode)) {
      return Response.json({ error: '仅预付款/后付款模式订单存在尾款' }, { status: 400 });
    }
    ...
    // 后付款：货款全额尾款（无预付款）；预付款：总额 − 预付款
    const isDeferred = order.payment_mode === 'deferred';
    const prepaymentJpy = isDeferred ? 0 : (parseFloat(order.prepayment_amount_jpy) || parseFloat(order.prepayment_amount) || 0);
    ...
    let surchargeRatePct = parseFloat(settingsMap[isDeferred ? 'deferred_payment_surcharge_rate' : 'pre_shipment_balance_surcharge_rate']);
