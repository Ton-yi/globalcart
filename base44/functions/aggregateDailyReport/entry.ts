...
function calculateDailySummary(orders, pools, allOrders, tierPurchases = []) {
    ...
    orders.forEach(order => {
        ...
        // 票务订单：下单阶段实付 = ticket_prepaid_total_jpy；普通订单使用 order_stage_payment_jpy
        const payment = isTicket ? (order.ticket_prepaid_total_jpy || order.paid_amount || 0) : (order.order_stage_payment_jpy || order.paid_amount || 0);
        // 票务订单退差价 = ticket_refund_jpy；普通订单 = refund_amount_jpy
        ...
        // 服务费：票务和普通订单都使用 service_fee_amount
